...
//手機版首頁輪播圖
$(function () {
    let divWidth = $('#sliderBoardRwd').width();
    let imgCount = $('#contentRwd li').length;

    // 複製第一張和最後一張圖片
    let firstClone = $('#contentRwd li:first').clone();
    let lastClone = $('#contentRwd li:last').clone();
    $('#contentRwd').append(firstClone);
    $('#contentRwd').prepend(lastClone);

    imgCount += 2; // 更新圖片數量以包括複製的圖片

    for(let i = 0; i < imgCount - 2; i++){
        $('#contentButtonRwd').append(`<li></li>`)
    }
    $('#contentButtonRwd li:first').addClass('clicked');

    $('#contentRwd li').width(divWidth); // li 的寬度
    $('#contentRwd').width(divWidth * imgCount); // ul的寬度
    $('#contentRwd').css('left', -divWidth); // 初始位置移動到第一張複製圖片的位置

    let index = 1;
    let timer = setInterval(moveToNext, 5000);

    $('#contentButtonRwd li').click(function(){
        clearInterval(timer); // 停掉計時器
        index = $(this).index() + 1; // 更新索引，+1 是因為複製圖片的存在
        ...
        $(this).addClass('clicked');
        $('#contentButtonRwd li').not(this).removeClass('clicked');
    ...
    function moveToNext() {
        ...
        $('#contentRwd').animate({
            ...
            if(index === imgCount - 1) { // 到達最後一張複製圖片
                $('#contentRwd').css('left', -divWidth); // 跳轉到第一張圖片
                index = 1;
            }
        });
        ...
        $(`#contentButtonRwd li:eq(${buttonIndex})`).addClass('clicked');
        $('#contentButtonRwd li').not(`:eq(${buttonIndex})`).removeClass('clicked');
    }
});
